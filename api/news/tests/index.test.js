const {getAvailableArticles, getArticle} = require('../index');


describe('Article module tests', function () {

  it('should extract article descriptions', async function () {
    let articles = await getAvailableArticles();

    expect(articles.length).toEqual(4);
  });

  it('should pare article page', async function () {
    let article = await getArticle('https://mic.scng.si/srecanje-upravnega-odbora-evropskega-zdruzenja-poklicnih-in-strokovnih-sol/');
